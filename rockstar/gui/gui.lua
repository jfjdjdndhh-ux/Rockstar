...
if game:GetService("CoreGui"):FindFirstChild("RockstarMenu") then game:GetService("CoreGui").RockstarMenu:Destroy() end
...
-- Сборщик Чекбоксов/Тумблеров (Включатели функций)
local function createToggleSetting(title, configKey, callback)
    ...
    row.BackgroundTransparency = 1 -- Для анимации появления
    ...
    -- Плавная анимация появления строки параметров
...
-- Автоматический подгрузчик модулей с гитхаба при клике на функцию в UI
local loadedModules = {}
...
-- ОТКРЫТИЕ НАСТРОЕК КОНКРЕТНОГО МОДУЛЯ (Правая сторона)
...
-- Перестроение списка функций по центру при смене вкладок
...
-- Первичный запуск на вкладку Visuals
